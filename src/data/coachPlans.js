...
export const SKYLAR_PUSH = {
  ...
  exercises: [
    ...
    },
    {
      ...
      order: 4,
      // One "set" here = one full myo-reps BLOCK (activation set +
      // mini-set cluster). Two blocks total; restSeconds is the rest
      // BETWEEN blocks. The 5–10s micro-rests inside a block are part
      // of the protocol in howTo and are never logged separately.
      sets: 2,
      repRange: '15–25',
      restSeconds: 90,
      ...
      secondaryMuscles: [],
      whyItMatters:
        'Myo-reps compress a whole workout\'s worth of effective reps into minutes. The activation set takes the side delt near failure — full fiber recruitment. The 5–10s micro-rests are too short for that state to fade, so EVERY rep of every mini-set is an effective rep. Perfect for small, fast-recovering muscles like side delts.',
      whyItMattersZh:
        'Myo-reps 把一整次训练的有效次数压缩进几分钟。激活组把中束打到接近力竭 = 肌纤维全员招募；5–10 秒微休息太短，这个状态不会消退 —— 所以之后每个迷你组的每一下都是有效次数。中束这种恢复快的小肌群用它效果最好。',
      howTo: [
        'ACTIVATION SET: 15–25 reps, stop 1–2 reps before failure.',
        'Rest ONLY 5–10 sec — dumbbells stay in your hands at your sides.',
        'MINI-SET: 3–5 reps. Then another 5–10 sec rest.',
        'Repeat mini-sets until you can\'t get 3 clean reps. That\'s ONE block — log it, take the full rest, then do block two.',
      ],
      howToZh: [
        '激活组：15–25 次，做到离力竭还剩 1–2 次。',
        '只休 5–10 秒 — 哑铃不放下，垂在体侧数五秒。',
        '迷你组：再做 3–5 次。然后再休 5–10 秒。',
        '重复迷你组，直到做不满 3 个标准次数为止。这算一个 block — 记录，完整休息 90 秒，再做第二个 block。',
      ],
      tips: [
        'Pick a weight where rep 20 gets genuinely hard — roughly half your cable-raise weight.',
        'Count each block\'s total (activation + all minis) as one logged set.',
        'Form break = block over. Momentum recruits traps.',
      ],
      tipsZh: [
        '选一个做到第 20 下会真吃力的重量 — 大约是绳索侧平举的一半。',
        '记录时一个 block（激活组 + 所有迷你组）算一组填进去。',
        '动作一变形这个 block 就结束 — 借力等于让斜方肌参与。',
      ],
      commonMistakes: [
        'Resting 30+ sec between mini-sets — recruitment resets and it becomes ordinary straight sets.',
        'Going heavy and turning it into an upright row.',
        'Putting the dumbbells down between mini-sets.',
      ],
      commonMistakesZh: [
        '迷你组之间休 30 秒以上 — 招募状态消退，就退化成普通组了。',
        '上重量太大变成直立划船。',
        '迷你组之间把哑铃放回架子上。',
      ],
      coachNote: 'The 90s rest timer = between blocks. Inside a block you count 5–10s in your head — never touch the timer.',
      coachNoteZh: '90 秒休息计时器只用在两个 block 之间。block 内部的 5–10 秒自己心里数 — 别碰计时器。',
      kneeFriendly: true,
      lowerBackFriendly: true,
    },
    {
